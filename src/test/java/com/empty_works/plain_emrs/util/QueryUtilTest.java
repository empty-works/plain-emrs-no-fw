...
class QueryUtilTest {
	...
	void testSelectWithOneCondition() {
		...
		qu.addField(new QueryField(QueryDataType.STRING, "user_email_address"));
		qu.setTable("users");
		qu.addCondition(new QueryCondition(QueryCondition.FIRST, "user_name=?", "dummy"));
		String finalQuery = qu.select();
	...
	void testSelectWithConditions() {
		...
		qu.addField(new QueryField(QueryDataType.STRING, "user_email_address"));
		qu.setTable("users");
		qu.addCondition(new QueryCondition(QueryCondition.FIRST, "user_home_address=?", "dummy1"));
		qu.addCondition(new QueryCondition(QueryCondition.AND, "user_email_address=?", "dummy2"));
		String finalQuery = qu.select();
