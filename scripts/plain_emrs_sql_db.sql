...
CREATE TABLE IF NOT EXISTS `plain_emrs`.`patients` (
  ...
  `patient_provider_id` VARCHAR(60) NOT NULL,
...
-- -----------------------------------------------------
-- Table `plain_emrs`.`tracked_patients`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `plain_emrs`.`tracked_patients` ;

CREATE TABLE IF NOT EXISTS `plain_emrs`.`tracked_patients` (
  `user_id` VARCHAR(75) NOT NULL,
  `patient_id` VARCHAR(60) NOT NULL,
  PRIMARY KEY (`user_id`),
  CONSTRAINT `fk_tracked_patients_users1`
    FOREIGN KEY (`user_id`)
    REFERENCES `plain_emrs`.`users` (`user_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE TABLE IF NOT EXISTS `plain_emrs`.`blood_relatives` (
  `medical_record_id` INT UNSIGNED NOT NULL,
  `user_id` VARCHAR(75) NOT NULL,
  `blood_relation` VARCHAR(45) NOT NULL,
  `alive_status` VARCHAR(45) NOT NULL DEFAULT 'Do not know',
  `deceased_age` INT NULL,
  `cause_of_death` BLOB NULL,
  PRIMARY KEY (`medical_record_id`, `user_id`),
